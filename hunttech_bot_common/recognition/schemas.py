...
DOCUMENT_SCHEMA: dict[str, Any] = {
    ...
    "properties": {
        ...
        "document_type": {
            "type": "string",
            "enum": ["CONTRACT", "ACT", "REPORT", "DECISION", "UPD", "INVOICE", "TASK", "RECEIPT", "ADDITIONAL_AGREEMENT", "OTHER", "UNKNOWN"],
        },
...
- Determine document_type primarily from the document TITLE/HEADING, not from incidental words. Examples: "ДОГОВОР...", "Договор возмездного оказания услуг", "Договор оказания услуг", "трудовой договор" → CONTRACT; "СЧЕТ №...", "Счет на оплату", "Счёт на оплату" → INVOICE; "АКТ..." → ACT; "УПД", "Универсальный передаточный документ", "Счет-фактура" → UPD; "Дополнительное соглашение", "Допсоглашение" → ADDITIONAL_AGREEMENT; "ОТЧЕТ №...", "Отчет об оказанных услугах", "Отчет о выполненных работах", "Отчет за июль..." → REPORT (это отчёт, отдельный тип документа, НЕ акт и НЕ договор); "РЕШЕНИЕ №...", "Решение о привлечении к ответственности...", "Решение о возврате/зачете...", "Решение СФР/ФНС/суда..." → DECISION (это решение, отдельный тип документа, НЕ акт и НЕ договор).
...
def _normalize_result(data: dict[str, Any], *, hint_text: str = "") -> dict[str, Any]:
    ...
    normalized["document_type"] = _normalize_enum(
        normalized.get("document_type"),
        {
            ...
            "дс": "ADDITIONAL_AGREEMENT",
            "appendix": "ADDITIONAL_AGREEMENT",
            "other": "OTHER",
        },
